...
## Транскрибатор собеседника (системный звук)
Новый режим пишет звук не с микрофона, а напрямую с устройства вывода (что вы слышите) и выводит расшифровку в отдельное окно.

1. Запустить `python meeting_transcriber.py`.
2. В консоли выбрать нужное loopback-устройство PyAudio (или оставить пустым, чтобы использовать вывод по умолчанию).
3. (Необязательно) выбрать loopback-устройство из списка `soundcard` — этот fallback используется автоматически, если PyAudio не работает с вашим железом.
4. В появившемся окне нажать «Начать прослушивание». Расшифровка будет появляться в текстовом поле с отметками времени.
5. Кнопка переключает режим прослушивания. При закрытии окна поток останавливается автоматически.

> ⚠ На Windows сначала используется PyAudio (WASAPI loopback). Если PyAudio не видит подходящие устройства, происходит автоматический переход на `soundcard`, который снимает выбранный вами loopback (или системный вывод по умолчанию). Убедитесь, что установлен пакет `soundcard` (он уже добавлен в requirements).
